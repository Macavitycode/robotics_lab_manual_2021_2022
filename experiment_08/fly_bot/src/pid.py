...
def PID(roll, pitch, yaw, f):
    # Define the global variables to prevent them from dying and resetting
    # to zero, each time a function call occurs. Some of these variables
    # may be redundant.

    global kp_roll, ki_roll, kd_roll, kp_pitch, ki_pitch, kd_pitch, kp_yaw
    global ki_yaw, kd_yaw, prevErr_roll, prevErr_pitch, prevErr_yaw, pMem_roll
    global pMem_yaw, pMem_pitch, iMem_roll, iMem_pitch, iMem_yaw
    global dMem_roll, dMem_pitch, dMem_yaw, flag, setpoint, sampleTime

    
    # Assign your PID values here. From symmetry, control for roll and pitch
    # is the same.
    kp_roll = 70
    ki_roll = 0.0002
    kd_roll = 89
    ...
    sampleTime = 0
    setpoint = 0
    err_pitch = float(pitch)*(180 / pi) - setpoint 
    err_roll = float(roll)*(180 / pi) - setpoint
    err_yaw = float(yaw)*(180 / pi) - setpoint
    currTime = time.time()
    ...
    # This is the Heart of the PID algorithm. PID behaves more accurately,
    # if it is sampled at regular intervals. You can change the sampleTime
    # to whatever value is suitable for your plant.
    ...
    # Limit the ESC pulses to upper limit and lower limit, in case the PID
    ...
    # Provide the motor velocities to the object 'f' that will now exit out of
    # this function, and gets published to gazebo, providing velocities to each
    # motor. Note that the sign here is +,-,+,- i.e CW, CCW, CW, CCW in
    # gazebo model. Change view of gazebo model (by scrolling) such that the
    # green line comes to your left, red line goes forward, and blue line goes
    # upward. This is the convention that i refer to as "Gazebo model" incase
    # you get confused.

    f.data = [fr_motor_vel,-fl_motor_vel,bl_motor_vel, -br_motor_vel]
    
    # Return these variables back to the control file.
    return err_roll, err_pitch, err_yaw, f
